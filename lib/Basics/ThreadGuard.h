...
struct ThreadGuard {
  ThreadGuard() = default;
  explicit ThreadGuard(size_t reserve) : threads(reserve) {}
  ~ThreadGuard() {
  ...
  auto emplace(Function&& f, Args&&... args) -> void {
    threads.emplace(std::forward<Function>(f), std::forward<Args...>(args...));
  }
  ...
  auto size() -> size_t { return threads.size(); }

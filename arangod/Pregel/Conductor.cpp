...
auto Conductor::_initializeWorkers(VPackSlice additional)
  ...
  if (ServerState::instance()->getRole() == ServerState::ROLE_SINGLE) {
    ...
    workers.emplace(server, std::make_unique<SingleServerConductorWorker>(
    ...
    for (auto const& [server, _] : vertexMap) {
      ...
      workers.emplace(server,
                      std::make_unique<ClusterConductorWorker>(
...
[[nodiscard]] auto ClusterConductorWorker::loadGraph(LoadGraph const& graph)
...
[[nodiscard]] auto SingleServerConductorWorker::loadGraph(
